...
// admin.html, first buyer gets it 75% off -- see is_golden_ticket in
...
async function notifyGoldenTicketClaimed({ productName, imageUrl, username, email, sessionId }) {
    ...
    const html = `
        <div style="font-family: -apple-system, sans-serif; max-width: 520px; margin: 0 auto;">
            ...
            <div style="border: 1px solid #eee; border-top: none; border-radius: 0 0 12px 12px; padding: 28px; background: #fff;">
                <p style="color: #333; font-size: 15px; line-height: 1.6; margin: 0 0 20px;">Someone just found and claimed the Golden Ticket -- this order gets that item 75% off. It's already been switched back to normal pricing for everyone else.</p>
    ...
    `;
    const text = `Golden Ticket claimed! "${productName}" went to ${username} (${email}) for 75% off. Order ref: ${sessionId}. View: ${orderUrl}`;
